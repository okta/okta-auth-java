...
import org.apache.shiro.event.support.EventClassComparator;
 ...
 * @since 1.3
 */
public class AuthenticationStateListenerComparator implements Comparator<AuthenticationStateListener> {

    //event class comparator is stateless, so we can retain an instance:
    private static final AuthenticationStateHandlerClassComparator EVENT_CLASS_COMPARATOR = new AuthenticationStateHandlerClassComparator();

    public int compare(AuthenticationStateListener a, AuthenticationStateListener b) {
        if (a == null) {
            if (b == null) {
                return 0;
            } else {
            ...
            if (a instanceof TypedAuthenticationStateListener) {
                TypedAuthenticationStateListener ta = (TypedAuthenticationStateListener)a;
                if (b instanceof TypedAuthenticationStateListener) {
                    TypedAuthenticationStateListener tb = (TypedAuthenticationStateListener)b;
